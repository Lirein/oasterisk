...
class AGI {
  ...
  function __construct($config = NULL, $optconfig = array()) {
    ...
    if(true == $this->config['phpagi']['error_handler']) {
    ...
    $this->channel = $this->request['agi_channel'];
    $this->uid = $this->request['agi_uniqueid'];
    $this->context = $this->request['agi_context'];
    $this->extension = $this->request['agi_extension'];
    $this->priority = $this->request['agi_priority'];
    $this->dnid = $this->request['agi_dnid'];
    $this->cidnum = $this->request['agi_callerid'];
    $this->cidname = $this->request['agi_calleridname'];
  
    // open audio if eagi detected
    if('1.0' == $this->request['agi_enhanced']) {
  ...
  function database_del($family, $key) {
    ...
    function database_get($family, $key) {
      $result = $this->evaluate("DATABASE GET \"$family\" \"$key\"");
      if(1 == $result['result']) {
        return $result['data'];
      }
    ...
    /**
     * Execures Log application to write log message
     *
     * @param string $level Режим журналирования: info, debug, notice, warging, error
     * @param string $text Текст сообщения об ошибке
     * @return array, see evaluate for return information. ['result'] is whatever the application returns, or -2 on failure to find application
     */
    function log($level, $text) {
      $text = str_replace(' ', '\ ', $text);
      return $this->evaluate("EXEC Log $level,$text");
    }
    ...
    function get_variable($variable, $getvalue = FALSE) {
      $variable = str_replace(' ', '\ ', str_replace("\n", '\n', addslashes($variable)));
    ...
    function fastpass_say_digits(&$buffer, $digits, $escape_digits = '') {
      ...
      if('' == $buffer || $proceed) {
        ...
        if(AGIRES_OK == $res['code'] && $res['result'] > 0) {
    ...
    function menu($choices, $timeout = 2000) {
      ...
      while (is_null($choice)) {
        foreach($choices as $prompt) {
          ...
          if(AGIRES_OK != $ret['code'] || -1 == $ret['result']) {
            $choice = -1;
            break;
          }
        ...
        foreach(explode('*', $result['result']) as $code) {
          if($command) {
            ...
            case '8':
              ...
              break;
            case '9':
              $text = explode(' ', $text);unset($text[count($text) - 1]);
              $text = join(' ', $text);
          ...
          $command = true;
            ...
            $text .= $code;
          } elseif('UPPERCASE' == $mode && isset($alpha['k'.$code])) {
            $text .= $alpha['k'.$code];
          } elseif('LOWERCASE' == $mode && isset($alpha['k'.$code])) {
            $text .= strtolower($alpha['k'.$code]);
          } elseif('SYMBOL' == $mode && isset($symbol['k'.$code])) {
            $text .= $symbol['k'.$code];
          }
        }
    ...
    function &new_AsteriskManager() {
      $this->asm = new AMI(NULL, $this->config);
      $this->asm->pagi = &$this;
    ...
    function evaluate($command) {
      ...
      while ($str = trim(fgets($this->in, 4096))) { //dummy read from other asterisk echo
      }
      ...
      $ret['result'] = -1;
      $ret['data'] = '';
      if(AGIRES_OK != $ret['code']) // some sort of error
      {
        $ret['data'] = $str;
      ...
      } else // normal AGIRES_OK response
      {
        ...
        for($i = 0; $i<strlen($str); $i++) {
          if($str[$i] == ' ') {
            if($i-$lastspace>1) {
              $token = substr($str, $lastspace+1, $i - $lastspace - 1);
              $ret['data'] .= ' '.$token;
              $lastspace = $i;
            }
            $lastspace = $i;
          ...
          if($str[$i] == '=') {
            ...
            $token = explode('=', substr($str, $lastspace+1, $i - $lastspace - 1));
            $ret[$token[0]] = $token[1];
            $lastspace = $i;
          } elseif($str[$i] == '(') {
            if($in_token==0) {
    ...
    function conlog($str, $vbl = 1) {
      ...
      if(false != $this->config['phpagi']['debug']) {
