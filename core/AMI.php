...
class AMI {
   ...
   */
  function send_request($action, $parameters = array(), $nowait = false) {
    $req = "Action: $action\r\n";
    $actionid = null;
  ...
  function wait_response($allow_timeout = false, $actionid = null) {
    ...
    if(isset($res['EventList']) && 'start' == $res['EventList']) {
      $evlist = array();
      do {
        $res = $this->wait_response(false, $actionid);
        if(isset($res['EventList']) && 'Complete' == $res['EventList']) {
          break;
        } else {
          $evlist[] = $res;
        }
    ...
    function connect($server = NULL, $username = NULL, $secret = NULL) {
      ...
      if(strrpos($server, ':') !== false) {
        if(strpos(substr($server, strrpos($server, ':')+1),']') === false) {
          $this->server = substr($server, 0, strrpos($server, ':'));
          $this->port = substr($server, strrpos($server, ':')+1);
        } else {
          $this->server = $server;
    ...
    function DBGet($family, $key, $actionid = NULL) {
      ...
      if(isset($response['Response']) && ("Success" == $response['Response'])) {
        $response = $this->wait_response(false, $actionid);
        return $response['Val'];
      } else if(isset($response['Event']) && ('DBGetComplete' == $response['Event'])) {
    ...
    /**
     * Gets channel variable value or null if variable or channel not exists
     *
     * @param string $channel Channel to read variable from
     * @param string $name Variable identifier or function call
     * @return string
     */
    function get_variable($channel, $name) {
      $data = $this->GetVar($channel, $name);
     ...
     *
     * Returns line-by-line config file with sections.
     *
     * @link http://www.voip-info.org/wiki-Asterisk+Manager+API+Action+GetConfig
     * @param string $filename Config filename
     * @param string $category Optional category id
     * @param string $actionid message matching variable
     */
    function ModuleCheck($module, $actionid = NULL) {
      $parameters = array('Module' => $module);
      if($actionid) {
        $parameters['ActionID'] = $actionid;
      }

      return $this->send_request('ModuleCheck', $parameters);
    }
     ...
     * @param string $actionid message matching variable
     */
    function GetConfig($filename, $category = NULL, $actionid = NULL) {
      ...
      return $this->send_request('GetConfig', $parameters);
    }
     ...
     * @param string $actionid message matching variable
     */
    function GetConfigJSON($filename, $category = NULL, $actionid = NULL) {
      $parameters = array('Filename' => $filename);
      if($actionid) {
        $parameters['ActionID'] = $actionid;
      }
      ...
      return $this->send_request('GetConfigJSON', $parameters);
    }
     ...
     * Get core channels
     *
     * @link http://www.voip-info.org/wiki-Asterisk+Manager+API+Core+Show+Channel
     */
    function CoreShowChannels($actionid = NULL) {
      ...
      return $this->send_request('CoreShowChannels', $parameters);
    }
     ...
     * CoreSettings
     *
     * @link http://www.voip-info.org/wiki-Asterisk+Manager+API+Action+CoreSettings
     */
