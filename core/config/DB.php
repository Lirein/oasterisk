 ...
 */
abstract class DB extends \Module {

  /**
   * Читает данные из базы данных Asterisk, возвращает объект
   *
   * @param string $path Путь в древовидной БД
   * @param string|object $json Описание структуры данных
   * @return null|array|object Возвращает массив или объект
   */
      ...
      foreach($json as $keyname => $keyvalue) {
        if(is_array($keyvalue)) {
          if(count($keyvalue)&&isset($keyvalue[0])) {
            ...
            for($i = 0; $i<$count; $i++) {
              if(is_string($keyvalue[0])) {
                $result->$keyname[] = self::getDB($path.'/'.$keyname, 'entry_'.($i+1));
              } else {
          ...
          if(is_array($keyvalue)&&is_array($data->$keyname)) {
            if(count($keyvalue)&&isset($keyvalue[0])) {
              ...
              foreach($data->$keyname as $value) {
                if(is_string($keyvalue[0])) {
                  $result &= self::setDB($path.'/'.$keyname, 'entry_'.($count+1), $value);
                } else {
  ...
  /**
   * Читает элемент с указанным номером из базы данных Asterisk, возвращает объект
   *
   * @param string $path Путь в древовидной БД
   * @param string $key Ключ хранящий идентификатор объекта
   * @param string $item Идентификатор объекта
   * @param string|object $json Описание структуры данных
   * @return null|array|object Возвращает массив или объект
   */
  public static function readDataItem($path, $key, $item, $json) {
    ...
    if($key) {
      ...
      if(!$found&&!is_array($json)) $result = null;
