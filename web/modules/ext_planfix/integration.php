...
class PlanfixModule extends \Module implements \module\IAGI, \module\IJSON {
...
//if(isset($map[$extid])) $extid=$map[$extid];
    ...
    if($eventtype == 'ACCEPTED') {
      ...
      $dialednum = $this->agi->get_variable('CHANNEL(peername)', TRUE);
    }
...
/*      $currentcaller=$this->agi->get_variable('CLRID', TRUE);
    ...
    if($eventtype == 'COMPLETED') {
      $extid = explode('&',$this->agi->get_variable('AEXT', TRUE));
      $callerid = $this->agi->get_variable('CLRID', TRUE);
      $duration = $this->agi->get_variable('ANSWEREDTIME', TRUE);
      if(empty($duration)) {
        $duration = $this->agi->get_variable('UNIQUEID', TRUE);
        $duration = time() - substr($duration, 0, strpos($duration, '.'));
      }
      $dialstatus = $this->agi->get_variable('DIALSTATUS', TRUE);
      $dialednum = $this->agi->get_variable('DIALEDPEERNAME', TRUE);
      $dialednuma = explode('/',$dialednum);
      $dialednum = array_pop($dialednuma);
      $dialednuma = explode('-',$dialednum);
      $dialednum = $dialednuma[0];
...
//      }
    ...
    foreach($extid as $ext) {
      ...
      if($eventtype=='ACCEPTED') {
        if($ext!=$dialednum) {
          ...
          $data = array('type' => $calltype, 'event'=>'COMPLETED', 'phone' => $clientnum, 'diversion' => $trunknum, 'ext' => $ext, 'callid' => $uniqueid, 'duration' => 0, 'status' => 'Cancelled', 'is_recorded' => 0);
        } else {
          $this->agi->verbose('Answer for '.$ext,3);
          $this->agi->set_variable('DIALEDPEERNUMBER', $dialednum);
          $this->agi->set_variable('DIALSTATUS', 'ANSWER');
          $this->agi->set_variable('_AEXT', $dialednum);
        ...
        if($eventtype=='COMPLETED') {
          ...
          if($ext!='') $sendaction = true;
        }
        $data = array('type' => $calltype, 'event'=>$eventtype, 'phone' => $clientnum, 'diversion' => $trunknum, 'ext' => $internalnum, 'callid' => $uniqueid)+$extdata;
      }

      if($ext!='') {
        $this->agi->verbose('Send '.$data['event'].' event as '.$calltype.' call. External phone '.$data['phone'].', Internal phone '.$data['ext'],3);
        if($eventtype == 'COMPLETED') {
    ...
    if(($eventtype=='ACCEPTED')&&(!$sendaction)) {
      if($calltype=='in') {
        $newtype = 'INCOMING';
        $clientnum = $callerid;
        $internalnum = $dialednum;
      } else {
        $newtype = 'OUTGOING';
        $internalnum = $callerid;
        $clientnum = $dialednum;
      ...
      $this->agi->set_variable('DIALSTATUS', 'ANSWER');
      $this->agi->set_variable('_AEXT', $dialednum);
      $data = array('type' => $calltype, 'event' => $newtype, 'phone' => $clientnum, 'diversion' => $trunknum, 'ext' => $internalnum, 'callid' => $uniqueid);
      $this->agi->verbose('Send '.$data['event'].' event as '.$calltype.' call. External phone '.$data['phone'].', Internal phone '.$data['ext'],3);
      $answer = $this->sendTelAPIRequest('event', $data);
      $data = array('type' => $calltype, 'event'=>$eventtype, 'phone' => $clientnum, 'diversion' => $trunknum, 'ext' => $internalnum, 'callid' => $uniqueid);
      $this->agi->verbose('Send '.$data['event'].' event as '.$calltype.' call. External phone '.$data['phone'].', Internal phone '.$data['ext'],3);
    ...
    if(($eventtype=='COMPLETED')&&(!$sendaction)) {
      if($calltype=='in') {
        $newtype = 'INCOMING';
        $clientnum = $callerid;
        $internalnum = $dialednum;
  ...
  public function sendRecord($request_data) {
  ...
  public function startRecord() {
    ...
    $this->agi->exec('MixMonitor',array('pf_'.$uniqueid.'.wav','','/usr/bin/ast2mp3 '.$uniqueid.' '.$login.' '.$passwd.' '.rawurlencode($localurl)));
    $filename = $this->agi->get_variable('MIXMONITOR_FILENAME', TRUE);
    $this->agi->set_variable('CDR(recordingfile)', $filename);
  ...
  public static function requestContact(string $contactId) {
    $request=array('contact' => array('id' => $contactId));
    return array('command' => 'contact.get', 'req' => $request);
  }
