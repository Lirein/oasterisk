...
class Auth extends \module\Subject {

  /**
   * Приватное свойство со ссылкой на класс реализующий интерфейс коллекции
   *
   * @var \pjsip\Auths $collection
   */
  public static $collection = 'pjsip\\Auths';
  ...
  public function save() {
    ...
    switch($this->data->auth_type) {
      case "md5": {
        $this->data->password = '';
        $this->data->refresh_token = '';
        $this->data->oauth_clientid = '';
        $this->data->oauth_secret = '';
      } break;
      case "google_oauth": {
        $this->data->md5_cred = '';
        $this->data->password = '';
