...
class Resource extends \module\Subject {

  /**
   * Приватное свойство со ссылкой на класс реализующий интерфейс коллекции
   *
   * @var \pjsip\Resources $collection
   */
  public static $collection = 'pjsip\\Resources';
  ...
  public function __construct(string $id = null) {
    ...
    if(isset($this->ini->$id)) {
      $v = $this->ini->$id;
      if(isset($v->type)&&($v->type=='resource_list')) {
        ...
        $this->interLock($id);
        $items = array();
        foreach($this->data->list_item as $item) {
          $entry = Endpoints::find($item);
          if($entry === null) $entry = Resources::find($item);
  ...
  public function save() {
    ...
    $this->ini->$entry->type='resource_list';
