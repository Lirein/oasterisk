...
class SIPDriverSettings extends \view\View {
  ...
  public static function getMenu() {
    return (object) array('name' => 'SIP', 'prio' => 1, 'mode' => 'expert', 'icon' => 'DialerSipSharpIcon');
  }
  ...
  public function json(string $request, \stdClass $request_data) {
    ...
    $ini = self::getINI('/etc/asterisk/sip.conf');
    switch($request) {
      case "get-codecs": {
        $returnData = array();
...
/*          var opt = document.createElement('option');
        ...
        setValue(avalue) {
          ...
          if((!isSet(avalue.id))&&(this.content.id == '')) avalue.id='domains-'+Math.random().toString(36).replace(/[^a-z]+/g, '').substr(2);
          if(isSet(avalue.id)) {
      ...
      function updateSIPCodecs() {
        this.sendRequest('get-codecs').success(function(data) {
          sip_codecs=data;
      ...
      function updateSIPTransports() {
        this.sendRequest('get-transports').success(function(data) {
          card.setValue({transport: {value: data, clean:true}});
        });
      }

      function updateSIPDriver() {
        this.sendRequest('sipdriver-get').success(function(data) {
          sip_driver=data;
          var codecs = [];
          ...
          if(sip_driver.allow) for(var i=0; i<sip_driver.allow.length; i++) {
            if(sip_driver.allow[i]=='all') {
              ...
              j=sip_codecs.indexOfId(sip_driver.allow[i]);
      ...
      function updateSIPTLS() {
        this.sendRequest('siptls-get').success(function(data) {
      ...
      function sendSIPTLS(sender, pass) {
        ...
        if(isSet(pass)) data.import.pass = pass;
        passdialog.setValue({pass: ''});
        this.sendRequest('siptls-set', data).success(function(ret) {
          if(ret) {
            tlsdialog.hide();
            passdialog.show();
          } else {
      ...
      function sendSIPTLSRequest(sender) {
        this.sendRequest('siptls-request').success(function(data) {
      ...
      function sendSIPDriverData(sender) {
        ...
        delete data.codecs;
        this.sendRequest('sipdriver-set', data).success(function() {
...
?>
      ...
      $(function () {
        ...
        obj = new widgets.checkbox(subcard1, {single: true, id: 'videosupport', value: false}, "Включить поддержку видео");
        ...
        obj = new widgets.input(subcard2,{id: 'externip', value: '0.0.0.0', pattern: /([0-9]{1,3}\\.){0,3}[0-9]{0,3}(|:[0-9]{0,5})/},_("Внешний IP (SIP)"));
