...
class ConfbridgeModule extends \Module implements \module\IAGI {
  ...
  public function agi(\stdClass $request_data) {
    ...
    if(isset($request_data->incoming)) { //incoming calls to confbridge processing (dialog)
      ...
      foreach($rooms as $roomname => $room) { //Scan room info
        if($room->active) {
          if(strlen($room->pin)>$require_room_pin) $require_room_pin=strlen($room->pin);
          foreach($room->users as $user) {
            ...
            if(($chantype == 'Local')&&(strpos($roomusernumber, '@staff-')!==false)) {
            ...
            if(($roomusernumber==$usernumber)||($roomusernumber==$channelnumber)) {
              $this->agi->verbose("User has by name found ".$usernumber,4);
              $has_room_user=true;
              if(strlen($user->pin)>$require_user_pin) $require_user_pin=strlen($user->pin);
            }
          }
          foreach($room->groups as $groupname) {
            ...
            foreach($groups->$groupname->users as $user) {
              ...
              if(($chantype == 'Local')&&(strpos($groupusernumber, '@staff-')!==false)) {
                if(!$abmodule) $abmodule = getModuleByClass('addressbook\AddressBook');
                if($abmodule) {
              ...
              if(($groupusernumber==$usernumber)||($groupusernumber==$channelnumber)) {
                $has_room_user=true;
                $this->agi->verbose("Group user has by name found ".$usernumber,4);
                if(strlen($user->pin)>$require_user_pin) $require_user_pin=strlen($user->pin);
              }
            }
          }
      ...
      $confroom=$this->agi->get_variable('EXTEN',true);
      ...
      while(true) {
        ...
        foreach($rooms as $roomname => $room) { //Search valid room
          if($room->active&&($room->pin==$roompin)) {
        ...
        if($has_room_user&&(strlen($userpin)||(($userpin=='')&&($require_user_pin>0)))) $has_room_user = false;
        if($roomenabled) break;
...
//        $this->agi->set_variable('CONFBRIDGE(user,wait_marked)','no');
  ...
  public function cancelPersistentRoom($roomid) {
    ...
    for($i=1; $i<=$user_cnt*2; $i++) {
      if(file_exists('/var/spool/asterisk/outgoing/room_'.$roomid.'_user_'.$i.'.call')&&(filemtime('/var/spool/asterisk/outgoing/room_'.$roomid.'_user_'.$i.'.call')>time())) {
        unlink('/var/spool/asterisk/outgoing/room_'.$roomid.'_user_'.$i.'.call');
      }
    }
    ...
    for($i=1; $i<=$group_cnt; $i++) {
      ...
      for($j=1; $j<=$grp_cnt; $j++) {
        if(file_exists('/var/spool/asterisk/outgoing/room_'.$roomid.'_group_'.$groupid.'_user_'.$j.'.call')&&filemtime('/var/spool/asterisk/outgoing/room_'.$roomid.'_group_'.$groupid.'_user_'.$j.'.call')>time()) {
          unlink('/var/spool/asterisk/outgoing/room_'.$roomid.'_group_'.$groupid.'_user_'.$j.'.call');
        }
      }
    }
  }
