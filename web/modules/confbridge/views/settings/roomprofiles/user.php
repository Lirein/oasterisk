...
class UserSettings extends \view\View {
  ...
  public function implementation() {
    ?>
      ...
      function loadUserProfile(profile_id) {
        this.sendRequest('user-profile', {user: profile_id}).success(function(data) {
          if(data.id==profile_id) {
            user_profile_id=data.id;
            rightsidebar_activate(user_profile_id);
            rightsidebar_init(sbdel, sbadd, sbselect);
            card.setValue(data);
            card.show();
      ...
      function sendUserProfileData() {
        ...
        this.sendRequest('set-user-profile', data).success(function() {
          user_profile_id=data.id;
          updateUserProfiles();
          return true;
        });
      ...
      function removeUserProfile() {
        showdialog('Удаление профиля','Вы уверены что действительно хотите удалить профиль участника?',"error",['Yes','No'],function(e) {
          if(e=='Yes') {
            ...
            this.sendRequest('remove-user-profile', data).success(function() {
              user_profile_id='';
              updateUserProfiles();
              return false;
            });
