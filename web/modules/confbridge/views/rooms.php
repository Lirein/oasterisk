...
class ConfbridgeManage extends \view\View {
      ...
      case "room": {
        ...
        if($confmodule&&self::checkEffectivePriv('confbridge_room', $request_data->id, 'system_info')) {
          ...
          foreach($return->users as $k => $v) {
            ...
            } elseif(strpos($return->users[$k]->number, '@staff-')!==false) {
          ...
          foreach($return->groups as $groupid => $group) {
            foreach($group->users as $runuserkey => $runuser) {
              ...
              } elseif(strpos($return->groups[$groupid]->users[$runuserkey]->number,'@staff-')!==false) {
  ...
  public function implementation() {
    ...
    <script>
      ...
      function updateRooms() {
        this.sendRequest('rooms').success(function(data) {
          var hasactive=false;
          var items=[];
          if(data.length) {
      ...
      function updateRoom() {
        ...
        if(id!=0) this.sendRequest('room', {id: id}).success(function(data) {
          if(data.number !== undefined) {
            rightsidebar_activate(id);
            ...
            for(var j = 0; j < data.groups.length; j++) {
              for(var i = 0; i < data.groups[j].users.length; i++) {
                ...
                var processtalk=true; //update for asterisk 13.21 or above
                if(isSet(data.groups[j].users[i].talk)) {
                   processtalk=false;
                }
                if(processtalk) {
      ...
      function kickUsers(users, room) {
        ...
        if(selectedusers.length>0) {
          this.sendRequest('kick', {room: room, users: selectedusers}).success(function() {
            return false;
          });
        }
      }
      ...
      function isolateUser(user) {
        ...
        this.sendRequest('isolate', {room: id, users: user}).success(function() {
          updateRoom();
          return false;
        });
      }
      ...
      function mergeRoomUsers() {
        ...
        if(selected) {
          this.sendRequest('merge', {room: id, users: mergeusers}).success(function() {
            unselectAll();
            updateRoomUsers();
            return false;
          });
      ...
      function unmuteGroup(groupid) {
        ...
        if(selected) {
          this.sendRequest('unmute', {room: id, users: isolateusers}).success(function() {
            updateRoom();
            return false;
          });
        }
      ...
      function muteGroup(groupid) {
        ...
        if(selected) {
          this.sendRequest('mute', {room: id, users: isolateusers}).success(function() {
            updateRoom();
            return false;
          });
        }
      ...
      function callGroup(groupid) {
        ...
        this.sendRequest('start', {room: id, users: callusers}).success(function() {
          updateRoom();
          return false;
        });
      }
      ...
      function lockRoom(btn) {
        ...
        if(!selected) {
          this.sendRequest(($(btn).hasClass('active')?'un':'')+'lock', {room: id}).success(function() {
            updateRoom();
            return false;
          });
        } else {
      ...
      function muteRoom(btn, user) {
        ...
        if(selected==0||(isSet(user))) {
          this.sendRequest(((btn=='un'||$(btn).hasClass('active'))?'un':'')+'mute', {room: id, users: [user]}).success(function() {
            updateRoom();
            return false;
          });
        } else {
          ...
          this.sendRequest(((muted==selected)?'un':'')+'mute', {room: id, users: isolateusers}).success(function() {
            updateRoom();
            unselectAll();
            return false;
          });
      ...
      function recordRoom(btn) {
        this.sendRequest(($(btn).hasClass('active')?'un':'')+'record', {room: id}).success(function() {
          updateRoom();
          return false;
        });
      }
      ...
      function inviteRoom(room, chan, user, conf_profile, profile, menu, trunknum, extendnum, extenddelay, callerid) {
        ...
        this.sendRequest('invite', {room: room, room_profile: conf_profile, profile: profile, menu: menu, channel: channel, callerid: callerid, extendnum: extendnum, delay: extenddelay}).success(function() {
          return false;
        });
      }
      ...
      function callRoomUser(user_id, group_id, channel) {
        if(!isSet(channel)) {
          this.sendRequest('call', {room: id, user_id: user_id, group_id: group_id}).success(function() {
            updateRoom();
            return false;
          });
        } else {
          this.sendRequest('call', {room: id, user_id: user_id, group_id: group_id, channel: channel}).success(function() {
            updateRoom();
            return false;
          });
        }
      ...
      function setPin(btn) {
        this.sendRequest('pin', {room: id, pin: $(btn).parent().prev().val()}).success(function() {
