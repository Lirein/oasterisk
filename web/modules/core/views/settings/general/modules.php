...
class ModulesSettings extends \view\View {
  ...
  public static function getMenu() {
    return (object) array('name' => 'Загружаемые модули', 'prio' => 10, 'icon' => 'ExtensionSharpIcon', 'mode' => 'expert');
  }
    ...
    switch($request) {
      ...
      case "set": {
        if(isset($request_data->autoload)&&self::checkPriv('settings_writer')) {
          $data = self::getINI('/etc/asterisk/modules.conf');
          $data->modules=array();
...
<?php } ?>
      ...
      function sendModules() {
        this.sendRequest('set', {autoload: $('#module-autoload').prop('checked'), data: readStateList('#module-list')}).success(function(data) {
