...
class BackupAndRestoreSettings extends \view\View {
  ...
  protected static function init() {
    ...
    if((self::$asterisketc==null)||(self::$asteriskdb==null)) {
      ...
      if((!self::$asterisketc)||(!self::$asteriskdb)) {
        $data = self::getINI('/etc/asterisk/asterisk.conf');
  ...
  public static function getMenu() {
    return (object) array('name' => 'Резервные копии', 'prio' => 13, 'icon' => 'SettingsBackupRestoreSharpIcon');
  }
      ...
      function upload(event, obj) {
        ...
        reader.onloadend = function(evt) {
          if(evt.target.readyState == FileReader.DONE) {
            this.sendRequest('upload', {data: evt.target.result}).success(function(data) {
              loadBackups();
              return true;
            });
          }
...
<?php } ?>
      ...
      function deleteBackup(name) {
        showdialog('Удалить резервную копию','Вы уверены что хотите удалить резервную копию?','error',['Yes','No'], function(result) { 
          if(result=='Yes') {
            this.sendRequest('remove', {codename: name}).success(function(data) {
              loadBackups();
              return true;
            });
          }
      ...
      function restoreConfig(name) {
        showdialog('Заменить конфигурацию','Вы уверены что хотите заменить текущую конфигурацию из файла резервной копии?','warning',['Yes','No'], function(result) { 
          if(result=='Yes') {
            this.sendRequest('restore', {codename: name}).success(function(data) {
              loadBackups();
              return true;
            });
          }
      ...
      function createBackup(obj) {
        ...
        if(!isSet(obj)) {
          ...
          if(codename.get(0).checkValidity()) {
            dialog.modal('hide');
            this.sendRequest('create', {codename: codename.val()}).success(function(data) {
              loadBackups();
              return true;
            });
          } else {
