...
class GenericCDR extends \view\View {
  ...
  public static function getLocation() {
    return 'statistics/generic';
  }

  public static function getMenu() {
    return (object) array('name' => 'Основной журнал', 'prio' => 0, 'icon' => 'oi oi-spreadsheet', 'mode' => 'expert');
  }
...
?>
      ...
      function cdr_state(state) {
        if(isSet(cdrstates[state])) return cdrstates[state];
        return state;
      }
      ...
      function processLevel(cdr) {
        var result=[];
        for(id in cdr) {
          var row = {src: '', dst: '', channel: '', state: '', calltime: null, timetotal: 0, timetalk: 0, recording: ''};
          ...
          row.timetotal=cdr[id].duration;
          row.timetalk=cdr[id].seconds;
          row.recording=cdr[id].record;
          if(isSet(cdr[id].value)) {
      ...
      function requestFile() {
        var data=card.getValue();
        this.sendRequest('file', {engine: data.engine, from: data.period.from, to: data.period.for}).success(function(data) {
          if(isSet(data.cdr)) {
...
//        obj.onClick=requestFile;
        obj = new widgets.table(card, {id: 'cdr', sorted: true, head: {src: _('src','Источник'), dst: _('dst','Назначение'), channel: _('channel','Канал назначения'), state: _('state','Состояние'), calltime: _('calltime','Время звока'), timetotal: _('duration','Длительность'), timetalk: null, recording: _('recording','Запись')}, value: [], clean: true});
