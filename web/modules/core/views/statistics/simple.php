...
class SimpleCDR extends \view\View {
...
?>
      ...
      function requestFile() {
        var data=card.getValue();
        this.sendRequest('file', {engine: data.engine, from: data.period.from, to: data.period.for}).done(function(data) {
          if(isSet(data.cdr)) {
...
//        obj.onClick=requestFile;
        obj = new widgets.table(card, {id: 'cdr', sorted: true, head: {src: _('src','Источник'), dst: _('dst','Назначение'), channel: _('channel','Канал назначения'), state: _('state','Состояние'), calltime: _('calltime','Время звока'), callend: _('callend','Окончание звонка'), recording: _('recording','Запись')}, value: [], clean: true});
