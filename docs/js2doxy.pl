...
sub debug_msg
{
    ...
    if( $level > 0 ) {
        foreach my $i (1 .. $level) {
            print( STDERR " " ) if( $opt_debug & $flag );
        }
    }
    print( STDERR "($level) ".$debug_names{$flag}." ($.): $msg\n" )
        if( $opt_debug & $flag );
}
...
sub next_token
{
    ...
    if( $cur_line ne "" )
    {
        ...
        if( $opt_debug & $DEB_SCANNER )
        {
            $_ = $token; 
            s/\n/\\n/go; 
            debug_msg( 0, $DEB_SCANNER, "'$_' ~ '$pattern' "
    ...
    if( $token ne "" )
    {
        ...
        elsif( $scan_mode == $S_STRING ) 
        {
            if( $token eq "\\" ) {
                ...
                syntax_err( "(comment/string) Unterminated string literal." )
                    if( ($token =~ /$newline_pattern/) && ($nextnl == 0) );
...
sub parse_doc_comment
{
    ...
    sub parse_type
    {
        ...
        if( /[\'\"]/ )
        {
            ...
            {
                syntax_err( "(comment) Unterminated string literal." )
    ...
    $doc->{text} .= "*/\n";
    debug_msg( 0, $DEB_PARSER, "Document for type "
...
sub parse_code
{
    ...
    debug_msg( 0, $DEB_PARSER, "'$token' ".$scan_mode_names[$scan_mode] );
    $token;
}
...
{
    my $context = shift;
    debug_msg( $context->{level}, $DEB_DETECTOR, "Find Variable in '".$context->{name}."'." );
    my $token = parse_code();
    ...
    if( $context->{otype} == $OT_FILE )
    {
        ...
        debug_msg( $context->{level}, $DEB_DATABASE, "Variable '$token'" );
        if( $last_doc )
        {
            if( $last_doc->{otype} == $OT_UNKNOWN )
            {
                ...
                else
                {
                    ...
                    debug_msg( $context->{level}, $DEB_PARSER, "Comment for variable '$token'." );
                }
            }
        }
    }
    debug_msg( $context->{level}, $DEB_DETECTOR, "Found Variable in '".$context->{name}."'." );
    $last_doc = undef;
    ";";
}
...
sub parse_this
{
    ...
    debug_msg( $context->{level}, $DEB_DETECTOR, "Find member variable in '"
        .$context->{name}."' with token '$token'." );

    if( $token =~ s/^this\.($identifier)$/\1/ )
    {
        my $nextToken = parse_code();
        if( $nextToken ne "=" )
        {
            if( $nextToken eq "}" ) {
                unshift( @parser_stack, $nextToken);
            }
            debug_msg( $context->{level}, $DEB_DETECTOR, "Not found member variable in '"
                .$context->{name}."' with token '$token'." );
            return;
        }
        ...
        if( not exists $context->{members}{$token} )
        {
            ...
            debug_msg( $context->{level}, $DEB_DATABASE, 
    ...
    debug_msg( $context->{level}, $DEB_DETECTOR, "Found Member variable in '"
        .$context->{name}."'." );
}
...
sub parse_function
{
    ...
    debug_msg( $context->{level}, $DEB_DETECTOR, "Find Function Definition in '"
        .$context->{name}."'." );
    ...
    if( $#token_queue > 1 && $token_queue[1] !~ /(?:^[{}=;]$|^\/\*|^\/\/)/ )
    {
        ...
        debug_msg( $context->{level}, $DEB_DETECTOR, "No Function Definition can follow '"
            .$token_queue[1]."'." );
        return;
    }
    ...
    debug_msg( $context->{level}, $DEB_DATABASE, "Added "
    ...
    if( $last_doc )
    {
        if( $last_doc->{otype} == $OT_UNKNOWN )
        {
            ...
            else
            {
                ...
                debug_msg( $context->{level}, $DEB_DATABASE, "Comment for function '$name'." );
            }
        }
        if( $last_doc->{otype} == $OT_CONSTRUCTOR )
        {
            ...
            else
            {
                ...
                debug_msg( $context->{level}, $DEB_DATABASE, "Comment for constructor '$name'." );
            }
        }
        $last_doc = undef;
    }
    ...
    debug_msg( $context->{level}, $DEB_DETECTOR, "End found Function $name Definition in '"
        .$context->{name}."'." );
...
sub parse_method
{
    ...
    debug_msg( $context->{level}, $DEB_DETECTOR, "Find Method $name Definition in '"
        .$context->{name}."'." );
    
    if( $#token_queue > 1 && $token_queue[1] !~ /(?:^[{}=;]$|^\/\*|^\/\/)/ )
    {
        #dump_struct( \@token_queue, "Stack" );
        #warning( "Method Definition cannot follow '".$token_queue[1]."'." );
        debug_msg( $context->{level}, $DEB_DETECTOR, "No Method Definition can follow '"
            .$token_queue[1]."'." );
        return;
    }
    ...
    my $fnContext;
    if( not exists $context->{members}{$name} )
    {
        $context->{members}{$name} = 
        { 
            name => $name,
            otype => $OT_MEMBERFUNC, 
            scope => $context,
        ...
        $fnContext = $context->{members}{$name};
    }
    else
    {
        $fnContext = $context->{members}{$name};
        syntax_err( "Method expected, found '".
                $object_type_names[$fnContext->{otype}]."'" )
            if(    $fnContext->{otype} != $OT_MEMBERFUNC);
        delete $fnContext->{unknown}
            if( exists $fnContext->{unknown} );
    }
    debug_msg( $context->{level}, $DEB_DATABASE, "Added "
        .( $name =~ /^\?/ ? "anonymous " : "" )."method '$name'." );
    if( $last_doc )
    {
        if( $last_doc->{otype} == $OT_UNKNOWN )
        {
            ...
            {
                $fnContext->{doc} = $last_doc;
                debug_msg( $context->{level}, $DEB_DATABASE, "Comment for method '$name'." );
            }
        }
        if( $last_doc->{otype} == $OT_FUNCTION )
        {
            if( exists $fnContext->{doc} )
            {
                warning( "Method comment for '$name'"
                    ." already exists, ignoring new." );
            }
            else
            {
    ...
    {
        next if( $token eq "," );
        syntax_err( "Method parameter name expected, found '$token'." )
            if( $token !~ /^$identifier$/ );
        push( @{$fnContext->{args}}, { name => $token } );
    }
    ...
    debug_msg( $context->{level}, $DEB_DETECTOR, "End found Method $name Definition in '"
        .$context->{name}."'." );
...
sub parse_constructor
{
    ...
    debug_msg( $context->{level}, $DEB_DETECTOR, "Find Constructor Definition in '"
        .$context->{name}."'." );
    ...
    if( $#token_queue > 1 && $token_queue[1] !~ /(?:^[{}=;]$|^\/\*|^\/\/)/ )
    {
        #dump_struct( \@token_queue, "Stack" );
        #warning( "Function Definition cannot follow '".$token_queue[1]."'." );
        debug_msg( $context->{level}, $DEB_DETECTOR, "No Constructor Definition can follow '"
            .$token_queue[1]."'." );
        return;
    }
    ...
    {
        next if( $token eq "," );
        syntax_err( "Constructor parameter name expected, found '$token'." )
            if( $token !~ /^$identifier$/ );
        push( @{$context->{args}}, { name => $token } );
    }
    ...
    delete $context->{objs};
    $context->{objs}{$_} = $objs->{$_} foreach ( keys %$objs );
    my $otlast = $context->{otype};
    $context->{otype}=$OT_CONSTRUCTOR;
    parse( $context );
    $context->{otype}=$otlast;
    delete $context->{objs};
    $context->{objs} = pop( @{$context->{symbol_stack}} );

    debug_msg( $context->{level}, $DEB_DETECTOR, "Found Constructor Definition in '"
        .$context->{name}."'." );
...
sub parse_class
{
    ...
    debug_msg( $context->{level}, $DEB_DETECTOR, "Find Class Definition in '"
        .$context->{name}."'." );
    ...
    if( $#token_queue > 1 && $token_queue[1] !~ /(?:^[{}=;]$|^\/\*|^\/\/)/ )
    {
        #dump_struct( \@token_queue, "Stack" );
        #warning( "Function Definition cannot follow '".$token_queue[1]."'." );
        debug_msg( $context->{level}, $DEB_DETECTOR, "No Class Definition can follow '"
            .$token_queue[1]."'." );
        return;
    }
    ...
    if( $token ne "{" )
    {
        syntax_err( "Class name expected, found '$token'." )
            if( $token !~ /^$identifier$/ );
        $name = $token;
        $token = parse_code();
    }
    ...
    {
        $fnContext = $context->{objs}{$name};
        syntax_err( "Class expected, found '".
                $object_type_names[$fnContext->{otype}]."'" )
    ...
    if( $last_doc )
    {
        if( $last_doc->{otype} == $OT_UNKNOWN )
        {
            ...
            {
                $fnContext->{doc} = $last_doc;
                debug_msg( $context->{level}, $DEB_DATABASE, "Comment for class '$name'." );
            }
        }
        if( $last_doc->{otype} == $OT_CONSTRUCTOR )
        {
    ...
    {
        $token = parse_code();
        $fnContext->{base} = $context->{objs}{$token};
        debug_msg( $context->{level}, $DEB_DETECTOR, "Class parent token '".$token."'.");
        $token = parse_code();
    }
    syntax_err( "'{' expected, found '$token'." )
        if( $token ne "{" );
    ...
    debug_msg( $context->{level}, $DEB_DETECTOR, "Found Class Definition in '"
        .$context->{name}."'." );
...
sub create_base
{
    ...
    if( not $scope )
    {
        ...
        $scope = $context;
        debug_msg( $context->{level}, $DEB_DATABASE, "Added missing base class '$base'." );
    }
    return $scope;
}
...
sub create_static_var
{
    ...
    $context->{members}{$member} = { otype => $OT_MEMBERSVAR };
    debug_msg( $context->{level}, $DEB_DATABASE, "Added static member variable '$member'." );
}
...
sub parse_prototype
{
    ...
    $_ = shift;
    debug_msg( $context->{level}, $DEB_DETECTOR, "Find Prototype Definition in '"
        .$context->{name}."' with token '$_'." );
    if( parse_code() ne "=" )
    {
        debug_msg( $context->{level}, $DEB_DETECTOR, "Not found Prototype Definition in '"
            .$context->{name}."' with token '$_'." );
        return;
    }
       ...
       && ( exists $context->{members}{$name} ))
    {
        ...
        debug_msg( $context->{level}, $DEB_DATABASE, "'$name' is a nested class." );
    }
    ...
    debug_msg( $context->{level}, $DEB_DATABASE, "'$name' is a class." );

    /^.+\.prototype/ && do
        {
            push( @parser_stack, "=" );
            parse_prototype( $fnContext, $_ );
            debug_msg( $context->{level}, $DEB_DETECTOR, "Found Nested Prototype Definition in '"
                .$context->{name}."'." );
            return;
        };
    /^.+\.fulfills$/ && do
        {
            ...
            debug_msg( $context->{level}, $DEB_DETECTOR, "Found Nested Interface Definition in '"
                .$context->{name}."'." );
            return;
        };
    ...
    if( $member eq "" )
    {
        ...
        debug_msg( $context->{level}, $DEB_DATABASE, "Set '$base' as base for class '$name'." );
        if( $last_doc )
        {
            if( exists $fnContext->{doc} )
            {
    ...
    else
    {
        ...
        if(( $token = parse_code()) =~ /^$identifier$/ )
        {
            my $base = $token;
            my $end = 1;
            # if( $base eq "async" ) { error_msg('test'); while( parse_code() ne ";" ) {} }
            if( $base eq "function" )
            {
            ...
            else
            {
                ...
                {
                    debug_msg( $context->{level}, $DEB_DATABASE, "'$member' is a "
                       .(  $scope->{otype} == $OT_MEMBERSVAR
                         ? "static " 
                         : "")."member "
                       .(  $scope->{otype} == $OT_MEMBERFUNC 
        ...
        if( $doc && exists $fnContext->{members}{$member})
        {
            ...
            else
            {
                ...
                debug_msg( $context->{level}, $DEB_DATABASE, "Comment for member '$member'." );
            }
        }
    }
    debug_msg( $context->{level}, $DEB_DETECTOR, "Found Prototype Definition in '"
        .$context->{name}."'." );
}
...
sub parse_interface
{
    ...
    $_ = shift;
    debug_msg( $context->{level}, $DEB_DETECTOR, "Find possible Interface in '"
        .$context->{name}."' with token '$_'." );
    ...
    or do
        {
            ...
            $last_doc = undef;
            debug_msg( $context->{level}, $DEB_DETECTOR, "Not found Interface in '"
                .$context->{name}."' with token '$_'." );
            return;
        };
    ...
    while(( $token = parse_code()) ne ")" )
    {
        ...
        debug_msg( $context->{level}, $DEB_DATABASE, "'$token' is an interface." );
        
        $fnContext->{fulfills}{$token} = $scope;
        debug_msg( $context->{level}, $context->{level}, $DEB_DATABASE, "'$name' implements '$token'." );
    }
    debug_msg( $context->{level}, $DEB_DETECTOR, "Found Interface in '".$context->{name}."'." );
}
...
sub parse
{
    ...
    PARSE: while(( $token = next_parser_token()) ne "" )
    {
        ...
        if( $scan_mode == $S_CODE )
        {
            for( $token )
            {
                ...
                /^{$/               
                    && do { ++$level; $context->{level} = $level; next; };
                /^var$/ && $level == 0
                ...
                /^null$/        
                    && do { next; };
                /^return$/        
                    && do { if($token && ($context->{otype} == $OT_CLASS)) {
                        $context->{otype} = $OT_UNKNOWN;
                        while(( $token = next_parser_token() ) == ';' ) {};
                ...
                /^await$/        
                    && do { next; };
                /^constructor$/        
                    && do { parse_constructor( $context ); next; };
                /^class$/ && $level == 0
                    && do { parse_class( $context ); next; };
                /^.+\.prototype/    
                    && do { parse_prototype( $context, $token ); next; };
                /^.+\.fulfills$/    
                    && do { parse_interface( $context, $token ); next; };
                /^.+$/    
                    && do { parse_method( $context, $token ) if($token && ($context->{otype} == $OT_CLASS)); next; };
            }
        }
        elsif( $scan_mode == $S_DOC_COMMENT )
        {
    ...
    syntax_err( "EOF found. '}' expected (level ".$level.")." ) 
...
sub generate_function
{
    ...
    $doc = $func->{doc} 
        if(   exists $func->{doc} 
           && (   $func->{otype} == $OT_FUNCTION
               || $func->{otype} == $OT_MEMBERFUNC || $func->{otype} == $OT_UNKNOWN ));
    $ctor = $func->{ctor}
        if(   exists $func->{ctor} 
           && (   $func->{otype} == $OT_CLASS
    ...
    if( $doc )
    {
        syntax_err( "Documentation for $npref$name is not for a function." )
            if( !((($doc->{otype} == $OT_FUNCTION) || ($doc->{otype} == $OT_MEMBERFUNC)) &&
                  (($func->{otype} == $OT_FUNCTION) || ($func->{otype} == $OT_MEMBERFUNC)))
               && $doc->{otype} != $OT_UNKNOWN );
        print( "\n".$doc->{text} );
...
sub generate_variable
{
    ...
    if( exists $var->{doc} )
    {
        syntax_err( "Documentation for $npref$name is not for a variable." )
            if( !((($var->{doc}{otype} == $OT_VARIABLE) || ($var->{doc}{otype} == $OT_MEMBERVAR)) &&
                  (($var->{otype} == $OT_VARIABLE) || ($var->{otype} == $OT_MEMBERVAR)))
