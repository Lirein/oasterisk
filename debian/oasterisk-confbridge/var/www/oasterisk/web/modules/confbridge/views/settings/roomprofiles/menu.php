...
class MenuSettings extends \core\ViewModule {
          ...
          if(isset($ini->$menu)) {
            ...
            if(isset($v->type)&&($v->type == 'menu')) {
              ...
              if($zonesmodule&&!$this->checkZones()) {
                $profile->zones = $zonesmodule->getObjectSeczones('confbridge_profile', $menu);
              }
      ...
      case "set-menu-profile": {
        if(isset($request_data->orig_id)&&self::checkEffectivePriv('confbridge_profile', $request_data->orig_id, 'settings_writer')) {
          ...
          if(isset($request_data->orig_id)&&($request_data->orig_id != '')&&($request_data->orig_id != $request_data->id)) {
            ...
            if($zonesmodule&&$this->checkZones()) {
              foreach($zonesmodule->getObjectSeczones('confbridge_profile', $orig_id) as $zone) {
                $zonesmodule->removeSeczoneObject($zone, 'confbridge_profile', $orig_id);
                $zonesmodule->addSeczoneObject($zone, 'confbridge_profile', $id);
              }
            }
          ...
          if((!isset($request_data->orig_id)) || $request_data->orig_id=='') {
            ...
            if($zonesmodule&&$this->checkZones()) {
              $eprivs = $zonesmodule->getCurrentPrivs('confbridge_profile', $id);
          ...
          if(findModuleByPath('settings/security/seczones')&&($zonesmodule&&!self::checkZones())) {
            $zones = $zonesmodule->getObjectSeczones('confbridge_profile', $id);
            foreach($zones as $zone) {
              $zonesmodule->removeSeczoneObject($zone, 'confbridge_profile', $id);
            }
      ...
      case "remove-menu-profile": {
        if(isset($request_data->id)&&self::checkPriv('settings_writer')) {
          ...
          if(isset($ini->$id)) {
            ...
            if(isset($v->type)&&($v->type == 'menu')) {
              if($zonesmodule) {
                foreach($zonesmodule->getObjectSeczones('confbridge_profile', $id) as $zone) {
                  $zonesmodule->removeSeczoneObject($zone, 'confbridge_profile', $id);
                }
              }
              unset($ini->$id);
    ...
    ?>
      ...
      widgets.confmenu=class confmenuWidget extends baseWidget {
          ...
          switch(action) {
            case "dialplan_exec": {
              ...
              if(!entry.contextselect.value) {
                var j=findById(entry.widget.context_data, list[0]);
            ...
            case "playback_and_continue": {
              ...
              if(!entry.soundselect.value) {
                var j=findById(entry.widget.sound_data, entry.hiddendata.value);
                var option = document.createElement('option');
                if(j==-1) {
        ...
        setValue(avalue) {
          ...
          if((typeof avalue.id == 'undefined')&&(this.content.id == '')) avalue.id='confmenu-'+Math.random().toString(36).replace(/[^a-z]+/g, '').substr(2);
          if(typeof avalue.id != 'undefined') {
            this.content.id=avalue.id;
            if(this.label) this.label.htmlFor=this.content.id;
          }
          if(typeof avalue.sound_data != 'undefined') {
      ...
      function loadMenu(profile_id) {
        sendRequest('menu-profile', {menu: profile_id}).success(function(data) {
            if(data.id==profile_id) {
              menu_profile_id=data.id;
              rightsidebar_activate('#sidebarRightCollapse', menu_profile_id);
              rightsidebar_init('#sidebarRightCollapse', sbdel, sbadd, sbselect);
              data.name=data.id;
      ...
      function sendMenuData() {
        ...
        sendRequest('set-menu-profile', data).success(function() {
          menu_profile_id=data.id;
          updateMenus();
          return true;
        });
      ...
      function removeMenu() {
        showdialog('Удаление профиля','Вы уверены что действительно хотите удалить профиль меню участника?',"error",['Yes','No'],function(e) {
          if(e=='Yes') {
            ...
            sendRequest('remove-menu-profile', data).success(function(data) {
              menu_profile_id='';
              updateMenus();
              return true;
            });
