...
class PersistentRoomSettings extends \core\ViewModule {
  ...
  public static function getZoneInfo() {
    ...
    $result->zoneClass = 'confbridge_room';
      ...
      case "persistent-room": {
        ...
        if(($confmodule)&&isset($request_data->id)&&self::checkEffectivePriv('confbridge_room', $request_data->id, 'settings_reader')) { 
          ...
          if($zonesmodule&&!self::checkZones()) {
            $return->zones=$zonesmodule->getObjectSeczones('confbridge_room', $request_data->id);
          }
      ...
      case "remove-persistent-room": {
        ...
        if(($confmodule)&&isset($request_data->id)&&self::checkPriv('settings_writer')) { 
          ...
          if($confmodule->removePersistentRoom($request_data->id)) {
            ...
            if($zonesmodule) {
              foreach($zonesmodule->getObjectSeczones('confbridge_room', $request_data->id) as $zone) {
                $zonesmodule->removeSeczoneObject($zone, 'confbridge_room', $request_data->id);
              }
            }
          } else {
      ...
      case "save-persistent-room": {
        ...
        if(($confmodule)&&isset($request_data->id)&&self::checkEffectivePriv('confbridge_room', $request_data->id, 'settings_writer')) { 
          if(isset($request_data->id)&&($request_data->id!='')&&($request_data->id!=$request_data->new_id)) {
            $confmodule->cancelPersistentRoom($request_data->id);
            if($zonesmodule&&self::checkZones()) {
              $zones = $zonesmodule->getObjectSeczones('confbridge_room', $request_data->id);
              foreach($zones as $zone) {
                $zonesmodule->removeSeczoneObject($zone, 'confbridge_room', $request_data->id);
                $zonesmodule->addSeczoneObject($zone, 'confbridge_room', $request_data->new_id);
              }
            }
          }
          if((!isset($request_data->id))||$request_data->id=='') {
            if($zonesmodule&&self::checkZones()) {
              $eprivs = $zonesmodule->getCurrentPrivs('confbridge_room', $request_data->new_id);
          ...
          if(findModuleByPath('settings/security/seczones')&&($zonesmodule&&!self::checkZones())) {
            $zones = $zonesmodule->getObjectSeczones('confbridge_room', $request_data->new_id);
            foreach($zones as $zone) {
              $zonesmodule->removeSeczoneObject($zone, 'confbridge_room', $request_data->new_id);
            }
  ...
  public function scripts() {
    ...
    ?>
      ...
      function updatePersistentRooms() {
        sendRequest('persistent-rooms').success(function(data) {
          var hasactive=false;
          var items=[];
          if(data.length) {
      ...
      function updatePersistentRoom() {
        if(persistent_room_id!=0) sendRequest('persistent-room', {id: persistent_room_id}).success(function(data) {
          if(typeof data.id != 'undefined') {
            rightsidebar_activate('#sidebarRightCollapse', data.id);
            rightsidebar_init('#sidebarRightCollapse', data.readonly?null:sbdel, (room_profiles.length == 0)?null:sbadd, sbselect);
      ...
      function removePersistentRoom() {
        showdialog('Удаление комнаты','Вы уверены что действительно хотите удалить постоянную конференц-комнату?',"error",['Yes','No'],function(e) {
          if(e=='Yes') {
            sendRequest('remove-persistent-room', {id: persistent_room_id}).success(function(data) {
      ...
      function savePersistentRoom() {
        ...
        if((persistent_room_id!='')&&(data.new_id!=persistent_room_id)) {
          showdialog('Наименование комнаты изменено','Выберите действие с комнатой:',"warning",['Rename','Copy', 'Cancel'],function(e) {
            ...
            if(proceed) {
              sendRequest('save-persistent-room', data).success(function() {
