...
class RoomSettings extends \core\ViewModule {
    ...
    switch($request) {
      ...
      case "set-room-profile": {
        if(isset($request_data->orig_id)&&self::checkEffectivePriv('confbridge_profile', $request_data->orig_id, 'settings_writer')) {
          $result = self::returnError('Неизвестная ощибка сохранения профиля');
          $ini = new \INIProcessor('/etc/asterisk/confbridge.conf');
          ...
          if((!isset($request_data->orig_id))||$request_data->orig_id=='') {
            ...
            if($zonesmodule&&self::checkZones()) {
              ...
              if($zone) {
                ...
                $result = self::returnError('danger', 'Отакзано в доступе');
                break;
              }
            }
          }
  ...
  public function scripts() {
    ?>
      ...
      function loadRoom(profile_id) {
        sendRequest('room-profile', {room: profile_id}).success(function(data) {
            if(data.id==profile_id) {
              room_profile_id=data.id;
              rightsidebar_activate('#sidebarRightCollapse', room_profile_id);
              rightsidebar_init('#sidebarRightCollapse', sbdel, sbadd, sbselect);
              card.setValue(data);
              card.show();
      ...
      function sendRoomData() {
        ...
        sendRequest('set-room-profile', data).success(function() {
          room_profile_id=data.id;
          updateRooms();
          return true;
        });
      ...
      function removeRoom() {
        showdialog('Удаление профиля','Вы уверены что действительно хотите удалить профиль конференц-комнаты?',"error",['Yes','No'],function(e) {
          if(e=='Yes') {
            ...
            sendRequest('remove-room-profile', data).success(function() {
              room_profile_id='';
              updateRooms();
              return false;
            });
