...
class RoomsSettings extends \core\MenuModule implements \JSONInterface {
  ...
  public function scripts() {
    ...
    <script>
      ...
      $(function () {
        ...
        obj=new widgets.select(sect, {id: 'chan', search: true}, "Абонент/Шлюз");
        ...
        obj=new widgets.input(sect, {id: 'extnum', pattern: '[A-D#*0-9wW]+'}, "Дополнительный номер", "Донабираемый внутренний номер сотрудника после события поднятия трубки");
      ...
      function dialogChannelChange(sender) {
        ...
        if(chandata[1] != user.login) {
          ...
          if(user.type == 'Local') {
            sendRequest('contact-property', {id: user.login}).success(function(data) {
              let values = {};
      ...
      function dialogSetUser(dialog, auser) {
        ...
        if(typeof user.mode == 'undefined') user.mode=(trunkNumber=='')?'peer':'trunk';
