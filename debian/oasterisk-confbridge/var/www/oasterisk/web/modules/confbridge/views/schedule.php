...
class ScheduleManage extends \core\ViewModule {
  ...
  public function scripts() {
    ...
    <script>
      ...
      function rescheduleRooms(aobject) {
        sendRequest('reschedule').success(function() {
          loadSchedule();
          return true;
        });
        dialog.hide();
      ...
      function rescheduleRoom(aobject) {
        sendRequest('reschedule', {roomid: aobject.getID(), date: datetime.getMoment().unix()+86400}).success(function() {
          loadSchedule();
          return true;
        });
      }
      ...
      function resetRooms(aobject) {
        sendRequest('reset').success(function() {
          loadSchedule();
          return true;
        });
        dialog.hide();
      ...
      function rescheduleUser(aobject, entry) {
        ...
        var groupid=user[1];
        var userid=user[2];
        sendRequest('reschedule', {date: datetime.getMoment().unix()+86400, roomid: aobject.labeltext, groupid: groupid, userid: userid}).success(function() {
          loadSchedule();
          return true;
        });
        return false;
      ...
      function loadSchedule() {
        sendRequest('schedule', {date: datetime.getMoment().unix()}).success(function(data) {
