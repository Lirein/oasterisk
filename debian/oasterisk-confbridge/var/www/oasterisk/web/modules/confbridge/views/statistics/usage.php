...
class UsageStatistics extends \core\ViewModule {
...
?>
      ...
      function loadGroups() {
        sendRequest('get-groups').success(function(data) {
          groups = data;
          if(roomdata) {
            drawCDR();
          }
      ...
      function drawCDR() {
        ...
        for(let i in roomdata.rooms) {
          ...
          if(typeof roomtogroup[i] == 'undefined') {
            roomtogroup[i] = index;
          ...
          for(let j in roomdata.periods) {
            count++;
            if(typeof datasets[count] == 'undefined') datasets[count] = [];
      ...
      function newGroup() {
        ...
        if(data.name!='') {
          ...
          sendRequest('add-group', {group: data.name, rooms: rooms}).success(function() {
            loadGroups();
          });
          chart.unselectAll();
          selectbtns();
...
//        obj.onClick=requestFile;
        ...
        ungroupbtn.onClick = async function() {
          ...
          if(result=='Yes') {
            let selected = chart.getSelected();
            for(let i in selected) {
              if(isgroup[selected[i].index]) sendRequest('remove-group', {group: chart.chart.data.labels[selected[i].index].substr(1).split(']')[0]}).success(function() {
                loadGroups();
              });
            }
          }
