<?php
  ...
  function oAsteriskAutoloader($class) {
    $csplit = explode("\\", $class);
    if((count($csplit)==2)&&($csplit[0]=='core')) $lclass=$csplit[1];
    else $lclass=$class;
    if(file_exists(dirname(__DIR__).'/asterisk/'.$lclass.'.php')) {
      require_once dirname(__DIR__).'/asterisk/'.$lclass.'.php';
    } else {
          ...
          if($file[0] != '.') {
            if(substr($file, -4)=='.php') { //if it is a PHP file
              ...
              foreach($newclasses as $class) {
                ...
                if($reflection->isSubclassOf('core\Module')) {
                  ...
                  if(!$reflection->isAbstract()) {
                    ...
                    while($parentclass = $parentclass->getParentClass()) {
                      ...
                      if($parentclassname != 'core\Module') $parent[] = $parentclassname;
                    }
                  }
