      ...
      while($row = $res->fetchArray(SQLITE3_ASSOC)) {
        ...
        if(isset($row['clid']))
          if(preg_match('/("(.+)"|([^ <>]+)|)\s*(<([a-z0-9.@_-]{0,})(>|)|)/',$row['clid'],$match)) {
            ...
            if(isset($match[5])) $rowdata->src->num=$match[5];
        ...
        $rowdata->record=isset($row['recordingfile'])?(empty($row['recordingfile'])?'':'/recording/'.rawurlencode(str_replace('/var/spool/asterisk/monitor/','',explode(',', $row['recordingfile'])[0]))):'';
