...
class INIProcessor implements Iterator {
  /**
...
//    //* TODO: Remove or comment
  ...
  /**
   * Читает параметры файла конфигурации и возвращает ассоциативный массив ключ-значение.
   ...
   * @return \stdClass Объект секциий - секция[]→ключ→значение
   */
  public function getDefaults($json) {
    ...
    foreach($json as $sectionName => $section) {
      if(isset($this->_sections[$sectionName])) {
        $result->$sectionName = $this->_sections[$sectionName]->getDefaults($section);
      } else {
  ...
  /**
   * Устанавливает значения INI файла, удаляя значения равные значениям по умолчанию
   *
   * @param mixed $json Массив или json строка
   * @param array $data Массив устанавливаемых секций со структурой ключ=значение
  ...
  public function setDefaults($json, $data) {
    ...
    foreach($data as $sectionName => $section) {
      if(isset($this->_sections[$sectionName]) && isset($json->$sectionName)) {
        $result &= $this->_sections[$sectionName]->setDefaults($json->$sectionName, $section);
      } else {
        $this->_sections[$sectionName] = new INISection($sectionName);
      }
    }
