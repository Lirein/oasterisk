...
class GenericCDR extends ViewModule {
  ...
  public static function getMenu() {
    return (object) array('name' => 'Основной журнал', 'prio' => 0, 'icon' => 'oi oi-spreadsheet');
  }
...
?>
      ...
      function requestFile() {
        var data=card.getValue();
        sendRequest('file', {engine: data.engine, from: data.period.from, to: data.period.for}).success(function(data) {
...
//        obj.onClick=requestFile;
        obj = new widgets.table(card, {id: 'cdr', head: {src: _('src','Источник'), dst: _('dst','Назначение'), channel: _('channel','Канал назначения'), state: _('state','Состояние'), calltime: _('calltime','Время звока'), timetotal: _('duration','Длительность'), timetalk: null, recording: _('recording','Запись')}, value: [], clean: true});
