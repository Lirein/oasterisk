...
class PlanfixSettings extends \core\ViewModule {
  ...
  public function scripts() {
    ...
    <script>
      ...
      function updateConfig() {
        ...
        sendRequest('get-config').success(function(data) {
      ...
      function sendConfig() {
        ...
        sendRequest('set-config', data).success(function() {
          updateConfig();
          return true;
        });
      }
      ...
      function addentry(issimple,list,key,value) {
        ...
        if(typeof key != 'undefined') inputs.get(0).value=key;
        if(typeof value != 'undefined') inputs.get(1).value=value;
        if(!issimple) {
          var small = node.find('small');
...
?>
      function getPasswd() {
        var props=$('#config-data');
        sendRequest('get-passwd', {user: props.find('#ami-user').val()}).success(function(data) {
