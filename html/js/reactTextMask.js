  ...
  */
    ...
    if (typeof mask === "string") {
      ...
      mask.forEach(function (character, index) {
        if (typeof character === "string") {
          ...
          if(src.slice(0,1)!='^') src = '^'+src;
          if(src.slice(src.length-1,src.length)!='$') src = src+'$';
          character.compile(src);
        }
      });
    }
  ...
  var MaskUtils = function MaskUtils(options) {
    ...
    this.isValueEmpty = function (value) {
      if(_this.maskOptions.mask instanceof RegExp) return (value==undefined)||((value.split("").length)==0);
    ...
    this.isValueFilled = function (value) {
      if(_this.maskOptions.lastEditablePosition == null) return false
    ...
    this.getDefaultSelectionForValue = function (value) {
      ...
      if(_this.maskOptions.mask instanceof RegExp) {
        return {
          start: filledLength,
          end: filledLength
        };
    ...
    this.getFilledLength = function (value) {
      if(_this.maskOptions.mask instanceof RegExp) return (value!=undefined)?(value.length):0;
    ...
    this.getLeftEditablePosition = function (position) {
      if(_this.maskOptions.mask instanceof RegExp) return 0;
    ...
    this.formatValue = function (value) {
      ...
      if(mask instanceof RegExp) {
        ...
        if(value == null) return null;
        let checkedPattern = 0;
        let checkedPostion = 0;
        let i = 0;
        while(i < value.length) {
          if(mask[checkedPattern] instanceof RegExp) {
            if(value.slice(checkedPostion, i+1) == '_') {
              checkedPattern++;
              checkedPostion=i+1;
            } else {
              if(mask.length>checkedPattern+1) {
                if(!value.slice(checkedPostion, i+1).match(mask[checkedPattern])) {
                  if(mask[checkedPattern+1] instanceof RegExp) {
                    checkedPattern++;
                    checkedPostion=i;
                    if(!(value.slice(checkedPostion, i+1).match(mask[checkedPattern]))&&!(value.slice(checkedPostion, i+1) == '_')) {
                      break;
                    }
                  } else if(typeof mask[checkedPattern+1] == "function") {
                    checkedPattern++;
                    checkedPostion=i;
                    if(!(mask[checkedPattern](value.slice(checkedPostion, i+1)))&&!(value.slice(checkedPostion, i+1) == '_')) {
                      break;
                    }
                  } else {
                    if((value.slice(i, i+1)!=mask[checkedPattern+1].slice(0, 1))&&!(value.slice(i, i+1) == '_')) {
                      break;
                    } else {
                      checkedPattern++;
                      checkedPostion=i;
          ...
          } else if(typeof mask[checkedPattern] == "function") {
            if(value.slice(checkedPostion, i+1) == '_') {
              checkedPattern++;
              checkedPostion=i+1;
            } else {
              if(mask.length>checkedPattern+1) {
                if((!mask[checkedPattern](value.slice(checkedPostion, i+1)))&&!(value.slice(checkedPostion, i+1) == '_')) {
                  if(mask[checkedPattern+1] instanceof RegExp) {
                    ...
                    if(value.slice(i, i+1)!=mask[checkedPattern+1].slice(0, 1)) {
                      break;
                    } else {
                      checkedPattern++;
                      checkedPostion=i;
            ...
            if(value.slice(checkedPostion, i+1)!=mask[checkedPattern].slice(0, i-checkedPostion+1)) {
              if(mask.length>checkedPattern+1) {
                if(mask[checkedPattern+1] instanceof RegExp) {
                  checkedPattern++;
                  checkedPostion=i;
        ...
        if(checkedPattern<mask.length) {
          if(checkedPostion==i) value = value+'_';
          checkedPattern++;
          while(checkedPattern<mask.length) {
            if(typeof mask[checkedPattern] == "string") {
              value = value+mask[checkedPattern];
            } else {
              value = value+'_';
            }
            checkedPattern++;
    ...
    this.processChange = function (currentState, previousState) {
      ...
      if(mask instanceof RegExp) {
        ...
        // } else if ((selection.end == previousSelection.start)&&(newValue!=previousValue)) {
        //   enteredString = newValue.substr(0, newValue.indexOf(previousValue));
        //   cursorPosition = 0;
        // }
        ...
        //     cursorPosition = deleteFromRight?(selection.start):(selection.start);
        //   }

        //   newValue = newValue.substr(0,cursorPosition) + newValue.substr(cursorPosition+removedLength);
        // }
        // newValue = newValue.substr(0,cursorPosition) + enteredString + newValue.substr(cursorPosition);
        // if(!newValue.match(mask)) {
        //   if(newValue != '') newValue = previousValue;
        // } else {
        //   cursorPosition += enteredString.length;
        // }
        if((!newValue.match(mask))||(_this.formatValue(newValue)!=newValue)) {
          if(newValue != '') {
            newValue = previousValue;
        ...
        while(i < newValue.length) {
          if(mask[checkedPattern] instanceof RegExp) {
            if(newValue.slice(checkedPostion, i+1) == '_') {
              checkedPattern++;
              checkedPostion=i+1;
            } else {
              if(mask.length>checkedPattern+1) {
                if(!newValue.slice(checkedPostion, i+1).match(mask[checkedPattern])) {
                  if(newValue.slice(i, i+1) == '_') {
                    newValue = newValue.slice(0,i) + newValue.slice(i+1, newValue.length);
                  }
                  if(mask[checkedPattern+1] instanceof RegExp) {
                    checkedPattern++;
                    checkedPostion=i;
                    if(!(newValue.slice(checkedPostion, i+1).match(mask[checkedPattern]))&&!(newValue.slice(checkedPostion, i+1) == '_')) {
                      newValue = previousValue;
                      break;
                    }
                  } else if(typeof mask[checkedPattern+1] == "function") {
                    checkedPattern++;
                    checkedPostion=i;
                    if(!(mask[checkedPattern](newValue.slice(checkedPostion, i+1)))&&!(newValue.slice(checkedPostion, i+1) == '_')) {
                      newValue = previousValue;
                      break;
                    }
                  } else {
                    if((newValue.slice(i, i+1)!=mask[checkedPattern+1].slice(0, 1))&&!(newValue.slice(i, i+1) == '_')) {
                      newValue = previousValue;
                      break;
                    } else {
                      checkedPattern++;
              ...
              if(mask.length>checkedPattern+1) {
                if((!mask[checkedPattern](newValue.slice(checkedPostion, i+1)))&&!(newValue.slice(checkedPostion, i+1) == '_')) {
                  ...
                  if(mask[checkedPattern+1] instanceof RegExp) {
                    ...
                    if(newValue.slice(i, i+1)!=mask[checkedPattern+1].slice(0, 1)) {
                      newValue = previousValue;
                      break;
                    } else {
                      checkedPattern++;
            ...
            if(newValue.slice(checkedPostion, i+1)!=mask[checkedPattern].slice(0, i-checkedPostion+1)) {
              if(mask.length>checkedPattern+1) {
                if(mask[checkedPattern+1] instanceof RegExp) {
                  checkedPattern++;
                  checkedPostion=i;
        ...
        if(i==selection.end-1) cursorPosition = i;
        else cursorPosition = selection.end;
      } else {
        if (selection.end > previousSelection.start) {
          enteredString = newValue.slice(previousSelection.start, selection.end);
    ...
    if(options.mask instanceof RegExp) {
      this.maskOptions = parseMask({mask: null, maskPlaceholder: null});
      this.maskOptions.mask = options.mask;
    } else {
  ...
  InputMask.propTypes = {
    ...
    mask: propTypes.oneOfType([propTypes.string, propTypes.instanceOf(RegExp), propTypes.arrayOf(propTypes.oneOfType([propTypes.string, propTypes.instanceOf(RegExp)]))]),
